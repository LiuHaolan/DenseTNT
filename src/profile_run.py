...
def demo_basic(rank, world_size, kwargs, queue):
    ...
    if world_size > 0:
        ...
        if args.recover_from_dir != None:
            # read the directory
            start_epoch = 11
            model.load_state_dict(torch.load("models.densetnt.1/model_save/model.11.bin"))
    ...
    if args.argoverse:
        ...
        train_dataloader = torch.utils.data.DataLoader(
            train_dataset, sampler=train_sampler, pin_memory=True,
            batch_size=args.train_batch_size // world_size,
            collate_fn=utils.batch_list_to_batch_tensors)

    # adjust num_train_epoch if read from ckpt
    num_train_epochs = int(args.num_train_epochs) - 3

    for i_epoch in range(num_train_epochs):
        
        # adding the offset
        i_epoch = i_epoch + start_epoch
...
def train_one_epoch_single(model, iter_bar, optimizer, device, args: utils.Args, i_epoch, optimizer_2=None):
    li_ADE = []
    li_FDE = []
    utils.other_errors_dict.clear()
    start_time = time.time()
    ...
    with torch.profiler.profile(schedule=torch.profiler.schedule(wait=2, warmup=2, active=6,repeat=1), on_trace_ready=torch.profiler.tensorboard_trace_handler('./log'), with_stack=True) as profiler:
...
def single_gpu_train(kwargs):
    args = kwargs['args']
    device = "cuda"
 #   model = torch.nn.ModuleList().to(device)
    model = VectorNet(args)
    model = model.to(device)

#    print([each[0] for each in model.named_parameters()])
    if 'set_predict' in args.other_params:
        optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=args.learning_rate)
    elif 'complete_traj-3' in args.other_params:
        optimizer = torch.optim.Adam(
            [each[1] for each in model.named_parameters() if not str(each[0]).startswith('decoder.complete_traj')],
            lr=args.learning_rate)
        optimizer_2 = torch.optim.Adam(
            [each[1] for each in model.named_parameters() if str(each[0]).startswith('decoder.complete_traj')],
            lr=args.learning_rate)
    else:
        optimizer = torch.optim.Adam(model.parameters(), lr=args.learning_rate)
    ...
    for i_epoch in range(int(args.num_train_epochs)):
        ...
        if 'complete_traj-3' in args.other_params:
            train_one_epoch_single(model, iter_bar, optimizer, device, args, i_epoch, optimizer_2=optimizer_2)
        else:
            train_one_epoch_single(model, iter_bar, optimizer, device, args, i_epoch)
...
def run(args):
    ...
    if args.distributed_training:
        ...
        # apply single gpy without multiprocess
        kwargs = {'args': args}
        single_gpu_train(kwargs)
