...
class ParallelNewSubGraph(nn.Module):

    def __init__(self, hidden_size, depth=None):
        super(ParallelNewSubGraph, self).__init__()
        if depth is None:
            depth = args.sub_graph_depth
 #       self.layers = nn.ModuleList([MLP(hidden_size, hidden_size // 2) for _ in range(depth)])
 ...
 #       self.layers_4 = nn.ModuleList([GlobalGraph(hidden_size) for _ in range(depth)])
    ...
    def forward(self, input_list: list):
        ...
        # parallel processing the input lists  
        batch_size = len(input_list)
        ...
        # from here start the input forwarding
        hidden_states = self.layer_0(hidden_states)
...
class VectorNet(nn.Module):
    ...
    def __init__(self, args_: utils.Args):
        ...
        self.point_level_sub_graph = ParallelNewSubGraph(hidden_size)
...
#        print('prep time: {} secs'.format(round(time.time() - starttime1, 2)))
        starttime1 = time.time()
 ...
 #       print('forward subgraph time: {} secs'.format(round(time.time() - starttime1, 2)))
        starttime1 = time.time()
...
#        print('rest time: {} secs'.format(round(time.time() - starttime2, 2)))

        return element_states_batch, lane_states_batch

    def forward_encode_sub_graph_haolan(self, mapping: List[Dict], matrix: List[np.ndarray], polyline_spans: List[List[slice]],
                                 device, batch_size) -> Tuple[List[Tensor], List[Tensor]]:
        """
        :param matrix: each value in list is vectors of all element (shape [-1, 128])
        :param polyline_spans: vectors of i_th element is matrix[polyline_spans[i]]
        ...
        # declare input_list_list as a NxMXL tensor
        # N: batch size
        # M: polyline length
        # L: feature nums of each polyline, 128 for argoverse
#        input_list_list = torch.zeros(batch_size, len(polyline_span) ,128)
 ...
 #       print('prep time: {} secs'.format(round(time.time() - starttime1, 2)))
     ...
     #   element_states_batch = self.point_level_sub_graph(input_list_list)
...
#        print('rest time: {} secs'.format(round(time.time() - starttime2, 2)))
    ...
    def forward(self, mapping: List[Dict], device):
        ...
        starttime1 = time.time()
        element_states_batch, lane_states_batch = self.forward_encode_sub_graph_haolan(mapping, matrix, polyline_spans, device, batch_size)
        ...
        res = self.decoder(mapping, batch_size, lane_states_batch, inputs, inputs_lengths, hidden_states, device)
        
#        print('total time: {} secs'.format(round(time.time() - starttime, 2)))
